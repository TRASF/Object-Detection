...
static uint16_t *display_buf = NULL; // buffer to hold data to be sent to display
...
#if DISPLAY_SUPPORT
  ESP_LOGI(TAG, "Free heap size before buffer allocation: %lu", esp_get_free_heap_size());
  if (display_buf == NULL) {
    // Try to allocate a smaller buffer for initial testing
    display_buf = (uint16_t *) heap_caps_malloc(96 * 2 * 96 * sizeof(uint16_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (display_buf == NULL) {
      ESP_LOGE(TAG, "Couldn't allocate display buffer");
      return kTfLiteError;
    }
  }
  ESP_LOGI(TAG, "Free heap size after buffer allocation: %lu", esp_get_free_heap_size());
#endif // DISPLAY_SUPPORT

#if ESP_CAMERA_SUPPORTED
  int ret = app_camera_init();
  if (ret != 0) {
    ESP_LOGE(TAG, "Camera init failed with error code: %d", ret);
    return kTfLiteError;
  }
